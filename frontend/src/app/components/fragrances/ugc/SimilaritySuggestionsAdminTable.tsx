...
import { setSimilaritySuggestionStatusAction } from '@/app/actions/fragranceUgcActions';
import {
  actionResultMessage,
  isSuccessMessage,
} from '@/app/actions/actionHelpers/fragranceActionHelpers';
import { MODERATION_STATUS_OPTIONS } from '@/app/utils/fragranceUgcComponentUtils';
import {
...
type SimilaritySuggestionsAdminTableProps = {
  items: SimilaritySuggestion[];
};
...
function fragranceTitle(fragrance: SimilaritySuggestion['fragrance']) {
  return `${fragrance.brand.name} / ${fragrance.name}`;
}

export default function SimilaritySuggestionsAdminTable({
  items,
}: SimilaritySuggestionsAdminTableProps) {
  ...
  const handleSave = (id: SimilaritySuggestion['id']) => {
    ...
    startTransition(() => {
      void (async () => {
        const result = await setSimilaritySuggestionStatusAction(null, formData);
  ...
  if (items.length === 0) {
    return (
      <div className={styles.emptyState}>{labels.noSimilaritySuggestions}</div>
    );
  }
          ...
          >
            <div className="flex flex-wrap items-center gap-3">
              <h2 className="text-lg font-semibold">
                {fragranceTitle(item.fragrance)} {'в†’'}{' '}
                {fragranceTitle(item.similar_fragrance)}
              </h2>
